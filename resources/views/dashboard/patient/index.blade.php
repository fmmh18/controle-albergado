...
@section('content')
@include('layouts.shared/page-title', ['sub_title' => 'Pacientes', 'page_title' =>  \Config::get('layout.titulo') ])

    <div class="row">

        <div class="card">
            <div class="card-body">
                {!! Form::open(['method' => 'get', 'autocomplete' => false, 'route' => ['dashboard.patient.index', http_build_query(Request::input())], 'class' => $errors->any() ? 'was-validated' : '', 'novalidate', "files" => true]) !!}
                <div class="row">
                ...
                <div class="col-12 text-end pt-3">
                    <a href="{!! route('dashboard.patient.create') !!}" class="btn btn-success rounded-0">Adicionar</a>
                    <button type="submit" class="btn btn-primary rounded-0"> Pesquisar</button>
                    <a href="{!! route('dashboard.patient.index') !!}" class="btn btn-secondary rounded-0">Limpar</a>
                </div>
                {!! Form::close() !!}
            </div>
        </div>
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        ...
                        <tr>
                            ...
                            <td>{!! $item->year !!}</td>
                            <td>{!! $item->origin !!}</td>
                            <td><a href="{!! route('dashboard.patient.show',$item->id) !!}" class="btn btn-warning"><i class="bi bi-pencil-square"></i></a></td>
                            <td><a href="{!! route('dashboard.patient.delete',$item->id) !!}" class="btn btn-danger btn-delete"><i class="bi bi-x-square"></i></a></td>
                        </tr>
                         @empty
                        <tr>
